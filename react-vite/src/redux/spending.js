...
export const thunkDeleteSpending = spendingId => async dispatch => {
    const res = await fetch(`/api/spendings/${spendingId}`, { method: 'DELETE' });
    if (res.ok) {
...
export default function spendingsReducer(state = initialState, action) {
    switch (action.type) {
        case LOAD_ALL_SPENDINGS:
            const allSpendings = {};
            action.spendings.forEach(spending => { allSpendings[spending.id] = spending });
            return { ...state, allSpendings };
        case LOAD_SPENDING_BY_ID:
            return { ...state, singleSpending: action.spending };
        case CREATE_SPENDING:
            return { ...state, allSpendings: { ...state.allSpendings, [action.spending.id]: action.spending } };
